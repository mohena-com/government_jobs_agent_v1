...
PY
        ...
        if [ "$READY" = "true" ]; then
            echo "QWEN: 6-slide plan ready for rendering"
            SUCCESS_QWEN=$((SUCCESS_QWEN + 1))
            printf '%s\n' "$PLAN" >> "$QWEN_LIST"
        else
            echo "QWEN: no complete 6-slide plan"
...
echo
echo "============================================================"
echo " DAILY GENERATION COMPLETE | V1.9.27"
echo "============================================================"
echo "Job Detail DOCX : ${JOB_COUNT}"
echo "6-slide plans: ${SUCCESS_QWEN}"
echo "No 6-slide plan: ${FAILED_QWEN}"
